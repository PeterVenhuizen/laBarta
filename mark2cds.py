					...
					if mark in trans:
						...
						for i, ex in enumerate(exs):
							...
							for j in xrange(0, 3):
								...
								try:
									...
									if ex_trs in m2star and len(ex_trs):

										#print ex, ex_trs

										if not start_found and not m2star.startswith(ex_trs):
											pass
											...
											if not end_found: 
												...
												#print track_len, len(ex_trs)+bool(leftover), len(ex_trs), ex_trs, leftover
												break
										...
										if not start_found:
											...
											# Causes problems for AT1G15740.1, AT1G36920.P1, AT2G47730.P1, AT3G14980.1 and AT4G10200.P1
											# were an upstream exon of the actual start starts with the same two or more amino acids
						...
						for i, ex in enumerate(exs):
							...
							for j in xrange(0, 3):
								...
								try: 
									#print t_id, i, ex, exons[g_id][t_id][ex]
									ex_trs, leftover = Sequence(exons[g_id][t_id][ex]).translate(j)
									...
									# Special case where the start codon is split over two exons
									elif any([not start_found and leftover == 'A', not start_found and leftover == 'AT']):
